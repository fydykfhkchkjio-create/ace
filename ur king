...
if isfile and isfile("STAR_HUB.json") then
    local ok, data = pcall(function() return HS:JSONDecode(readfile("STAR_HUB.json")) end)
...
if isfile and isfile("STAR_HUB.json") then
    local ok2, d2 = pcall(function() return HS:JSONDecode(readfile("STAR_HUB.json")) end)
...
-- STAR HUB extra state (single table to stay under the local limit)
local STAR={medusaReset=false,antiKick=false,brainrot=false,tpBat=false,batV2=false,tpConn=nil,v2Conn=nil,v2Rot=nil,hitCD=false,v2CD=false}
...
local MOB_POS_FILE="STAR_BtnPos.json"
...
local function setupSpeedIndicator(char)
    ...
    local discordLabel=Instance.new("TextLabel",bb);discordLabel.Size=UDim2.new(1,0,0.4,0);discordLabel.BackgroundTransparency=1;discordLabel.Text="script by Sultan "
...
-- ANTI KICK (from STAR)
-- ============================================================
function STAR.enableAntiKick()
    STAR.antiKick=true
    task.spawn(function()
        while STAR.antiKick do
            task.wait(0.5)
...
function STAR.disableAntiKick() STAR.antiKick=false;STAR.brainrot=false end
...
queueAutoBatStart=function()
    if STAR.antiKick and STAR.brainrot then return end
    if autoLeftEnabled then autoLeftEnabled=false;if autoLeftSetVisual then autoLeftSetVisual(false) end;stopAutoLeft() end
    if autoRightEnabled then autoRightEnabled=false;if autoRightSetVisual then autoRightSetVisual(false) end;stopAutoRight() end
    startBatAimbot()
end
...
local function resetAllSettings()
    ...
    STAR.medusaReset=false;STAR.disableAntiKick()
...
-- ============================================================
local STAR_SLAP_LIST={"Bat","Slap","Iron Slap","Gold Slap","Diamond Slap","Emerald Slap","Ruby Slap","Dark Matter Slap","Flame Slap","Nuclear Slap","Galaxy Slap","Glitched Slap"}
function STAR.findBat()
    local char=LP.Character; if not char then return nil end
    for _,name in ipairs(STAR_SLAP_LIST) do local t=char:FindFirstChild(name); if t and t:IsA("Tool") then return t end end
    local bp=LP:FindFirstChildOfClass("Backpack")
    if bp then
        for _,name in ipairs(STAR_SLAP_LIST) do
...
-- ===== TP BAT =====
function STAR.tpHit()
    if STAR.hitCD then return end
    STAR.hitCD=true
    pcall(function()
        local bat=STAR.findBat()
        if bat then
            bat:Activate()
    ...
    task.delay(0.08,function() STAR.hitCD=false end)
end
function STAR.startTPBat()
    if STAR.tpConn then return end
    STAR.tpBat=true
    STAR.tpConn=RunService.Heartbeat:Connect(function()
        if not STAR.tpBat then return end
        local char=LP.Character; if not char then return end
        local hrp=char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
        local tr=STAR.closestRoot()
        if tr then
...
function STAR.stopTPBat()
    if STAR.tpConn then STAR.tpConn:Disconnect();STAR.tpConn=nil end
    STAR.tpBat=false
end
-- ===== BAT V2 =====
function STAR.v2Swing()
    if STAR.v2CD then return end
    STAR.v2CD=true
    pcall(function()
        local char=LP.Character; if not char then return end
        local bat=STAR.findBat()
    ...
    task.delay(0.35,function() STAR.v2CD=false end)
end
function STAR.startBatV2()
    if STAR.v2Conn then STAR.v2Conn:Disconnect() end
    STAR.batV2=true
    local hum0=LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum0 then
        if STAR.v2Rot==nil then STAR.v2Rot=hum0.AutoRotate end
        hum0.AutoRotate=false
    end
    STAR.v2Conn=RunService.RenderStepped:Connect(function()
        if not STAR.batV2 then return end
        local char=LP.Character; if not char then return end
        ...
        local target,targetDist=STAR.closestRoot()
        ...
        if targetDist<=8 then STAR.v2Swing() end
    end)
end
function STAR.stopBatV2()
    STAR.batV2=false
    if STAR.v2Conn then STAR.v2Conn:Disconnect();STAR.v2Conn=nil end
    STAR.v2CD=false
    ...
    if hum then
        hum.AutoRotate=(STAR.v2Rot==nil) and true or STAR.v2Rot
...
-- ============================================================
-- MOBILE BUTTONS  (STAR stack-button style)
-- ============================================================
local function destroyMobileButtons()
    if mobGuiRef then pcall(function() mobGuiRef:Destroy() end);mobGuiRef=nil end
    for _,n in ipairs({"STARMobileButtons","SpectrumMobileButtons","MoveeMobileButtons"}) do
        local old=game:GetService("CoreGui"):FindFirstChild(n);if old then old:Destroy() end
        local pgui=LP:FindFirstChild("PlayerGui");if pgui then local o=pgui:FindFirstChild(n);if o then o:Destroy() end end
    end
    mobBtnRefs={}
...
local function buildMobileButtons()
    ...
    mobGui.Name = "STARMobileButtons"; mobGui.ResetOnSpawn = false; mobGui.DisplayOrder = 15; mobGui.IgnoreGuiInset = true
    ...
    local _, refTPBat = makeMobileBtn("TP\nBAT", 2, 3, true, function(on)
        if on then STAR.startTPBat() else STAR.stopTPBat() end
        if STAR.setTPBatVisual then STAR.setTPBatVisual(on) end
    end)
    mobBtnRefs["tpBat"] = refTPBat

    local _, refBatV2 = makeMobileBtn("BAT\nV2", 0, 1, true, function(on)
        if on then STAR.startBatV2() else STAR.stopBatV2() end
        if STAR.setBatV2Visual then STAR.setBatV2Visual(on) end
    end)
    ...
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
    if mobBtnRefs.tpBat then mobBtnRefs.tpBat(STAR.tpBat) end
    if mobBtnRefs.batV2 then mobBtnRefs.batV2(STAR.batV2) end
end
...
-- ============================================================
pcall(function()
    if not(isfile and isfile("STAR_HUB.json")) then return end
    local ok,d=pcall(function() return HS:JSONDecode(readfile("STAR_HUB.json")) end)
    ...
    if type(d.medusaReset)=="boolean" then STAR.medusaReset=d.medusaReset end
    if type(d.antiKick)=="boolean" then STAR.antiKick=d.antiKick end
...
pcall(function()
    ...
    -- Anti Kick
    if STAR.antiKick then
        task.spawn(function() task.wait(1); STAR.antiKick=false; STAR.enableAntiKick(); if STAR.setAntiKickVisual then STAR.setAntiKickVisual(true) end end)
    end
    -- Medusa Reset
    if STAR.medusaReset then
...
;(function()
    ...
    GuiHub.Name="STARHub"; GuiHub.ResetOnSpawn=false
    ...
    TL.Text="STAR HUB"; TL.TextColor3=C.text; TL.TextSize=17; TL.Font=Enum.Font.GothamBlack
    ...
    ML.Text="STAR HUB • PREMIUM"; ML.TextColor3=C.textDim; ML.TextSize=10; ML.Font=Enum.Font.GothamBold
    ...
    MiniBtn.Text="STAR HUB"; MiniBtn.TextColor3=C.text; MiniBtn.Font=Enum.Font.GothamBlack; MiniBtn.TextSize=11
...
;(function()
    ...
    local _,svMedReset=addToggleRow(cp,"Medusa Reset",STAR.medusaReset,8,nil,function(on)
        STAR.medusaReset=on
        if on then setupMedusa(LP.Character) elseif not medusaCounterEnabled then stopMedusaCounter() end
        saveConfig()
    end)
    STAR.setMedusaResetVisual=svMedReset
    addSectLbl(cp,"PROTECTION",13)
    local _,svAntiKick=addToggleRow(cp,"Anti Kick",STAR.antiKick,14,nil,function(on)
        if on then STAR.antiKick=false; STAR.enableAntiKick() else STAR.disableAntiKick() end
        saveConfig()
    end)
    STAR.setAntiKickVisual=svAntiKick
    addSectLbl(cp,"TP BAT / BAT V2",15)
    local _,svTPBat=addToggleRow(cp,"TP Bat",STAR.tpBat,16,nil,function(on)
        if on then STAR.startTPBat() else STAR.stopTPBat() end
        if mobBtnRefs.tpBat then mobBtnRefs.tpBat(on) end
    end)
    STAR.setTPBatVisual=svTPBat
    local _,svBatV2=addToggleRow(cp,"Bat V2",STAR.batV2,17,nil,function(on)
        if on then STAR.startBatV2() else STAR.stopBatV2() end
        if mobBtnRefs.batV2 then mobBtnRefs.batV2(on) end
    end)
    STAR.setBatV2Visual=svBatV2
...
if STAR.medusaReset then setupMedusa(LP.Character) end
if STAR.antiKick then STAR.antiKick=false; STAR.enableAntiKick() end
CandyApplyCustomSky(currentSkyTheme)
buildMobileButtons()

end)() -- end GUI function

print("STAR HUB LOADED")
